...
public class EmpleadoExporterPDF {
	...
	private void escribirCabeceraDeLaTabla(PdfPTable tabla) {
		...
		celda.setPhrase(new Phrase("Condición", fuente));
		tabla.addCell(celda);
	...
	private void escribirDatosDeLaTabla(PdfPTable tabla) {
		for (Empleado empleado : listaEmpleados) {
			...
			tabla.addCell(empleado.getNombreCompleto());
			tabla.addCell(empleado.getNombreCorto());
			...
			tabla.addCell(String.valueOf(empleado.getCondicion()));
            tabla.addCell(String.valueOf(empleado.getTipoEstado()));
		}
	}
	...
	public void exportar(HttpServletResponse response) throws DocumentException, IOException {
		...
		tabla.setWidths(new float[] { 1f, 2.3f, 2.3f, 6f, 2.9f, 3.2f, 2f, 2.2f, 2.2f });
